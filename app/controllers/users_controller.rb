class UsersController < ApplicationController
  ...
  def destroy_multiple
      if params[:user_ids]
        ...
        message = {notice: 'Tutor borrado con éxito.'}
      else
        message = {alert: 'Por favor selecciona al menos un tutor.'}
      end

      respond_to do |format|
        format.html { redirect_to users_url, message }
        format.json { head :no_content }
      end
  end
  ...
  private
    ...
    def user_params
      params.require(:user).permit(:usuario, :password, :admin, :nombre, :apellido_materno, :apellido_paterno, :email, :socio, :organizacion, :pais, :estado, :municipio)
    end
end
